...
TEST(BST_InOrder, InsertDuplicate) {
    BST_InOrder<int> bst;
    EXPECT_TRUE(bst.insert(10).second);
    EXPECT_FALSE(bst.insert(10).second);
    EXPECT_EQ(bst.size(), 1u);
}

TEST(BST_InOrder, IteratorEquality) {
    BST_InOrder<int> bst;
    bst.insert(1);
    auto it1 = bst.begin();
    auto it2 = bst.begin();
...
TEST(BST_InOrder, ClearEmptyTree) {
    BST_InOrder<int> bst;
    bst.clear();
    EXPECT_TRUE(bst.empty());
}
...
TEST(BST_PreOrder, SortedInsert) {
    BST_PreOrder<int> bst;
